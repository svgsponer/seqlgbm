...
template <typename, typename, typename T, typename> class DoubleArrayImpl {
  public:
    ...
    // In searching dictionaries, the values associated with the matched keys
    // are stored into or returned as <result_type>s.
    ...
    // <DoubleArrayImpl> has 2 kinds of set_result()s. The 1st set_result() is
    // to set a value to a <value_type>. The 2nd set_result() is to set a value
    // and a length to a <result_pair_type>. By using set_result()s, search
    // methods can return the 2 kinds of results in the same way. Why the
    // set_result()s are non-static? It is for compatibility.
    //
    ...
    // Note that the key-value pairs must be arranged in key order and the
    // values must not be negative. Also, if there are duplicate keys, only the
    // first pair will be stored in the resultant dictionary. `progress_func' is
    // a pointer to a callback function. If it is not NULL, it will be called in
    // build() so that the caller can check the progress of dictionary
    // construction. For details, please see the definition of
    ...
    // from the file. `offset' specifies the number of bytes to be skipped
    // before reading an array. `size' specifies the number of bytes to be read
    // from the file. If the `size' is 0, the whole file will be read. open()
    // returns 0 iff the operation succeeds. Otherwise, it returns a non-zero
    // value or throws a <Darts::Exception>. The exception is thrown when and
    // only when a memory allocation fails.
    ...
    // `node_pos' specifies the start position of matching. This argument
    // enables the combination of exactMatchSearch() and traverse(). For
    // example, if you want to test "xyzA", "xyzBC", and "xyzDE", you can use
    // traverse() to get the node position corresponding to "xyz" and then you
    // can use exactMatchSearch() to test "A", "BC", and "DE" from that
    // position. Note that the length of `result' indicates the length from the
    // `node_pos'. In the above example, the lengths are { 1, 2, 2 }, not { 4,
    // 5, 5 }.
    template <class U>
...
int DoubleArrayImpl<A, B, T, C>::open(const char *file_name, const char *mode,
    ...
    if (size > 256) {
        if (std::fread(buf + 256, unit_size(), size - 256, file) !=
            size - 256) {
            std::fclose(file);
            delete[] buf;
            return -1;
...
template <typename A, typename B, typename T, typename C>
int DoubleArrayImpl<A, B, T, C>::save(const char *file_name, const char *mode,
...
inline U DoubleArrayImpl<A, B, T, C>::exactMatchSearch(
    const key_type *key, std::size_t length, std::size_t node_pos) const {
    U result;
...
template <typename T> class AutoPool {
  public:
...
template <typename T> void AutoPool<T>::resize_buf(std::size_t size) {
    std::size_t capacity;
    if (size >= capacity_ * 2) {
        capacity = size;
    } else {
...
template <typename T> class AutoStack {
  public:
    AutoStack() : pool_() {}
    ~AutoStack() { clear(); }
...
template <typename T> class Keyset {
  public:
...
class DawgNode {
  public:
    DawgNode()
        : child_(0), sibling_(0), label_('\0'), is_state_(false),
          has_sibling_(false) {}
...
class DawgBuilder {
  public:
    DawgBuilder()
        : nodes_(), units_(), labels_(), is_intersections_(), table_(),
          node_stack_(), recycle_bin_(), num_states_(0) {}
    ~DawgBuilder() { clear(); }
...
class DoubleArrayBuilder {
  public:
    ...
    template <typename T> void build(const Keyset<T> &keyset);
  ...
  private:
    ...
    void build_from_dawg(const DawgBuilder &dawg, id_type dawg_id,
                         id_type dic_id);
    id_type arrange_from_dawg(const DawgBuilder &dawg, id_type dawg_id,
                              id_type dic_id);

    template <typename T> void build_from_keyset(const Keyset<T> &keyset);
    template <typename T>
...
template <typename T> void DoubleArrayBuilder::build(const Keyset<T> &keyset) {
...
void DoubleArrayBuilder::build_from_keyset(const Keyset<T> &keyset,
                                           ...
                                           std::size_t depth, id_type dic_id) {
...
template <typename T>
id_type
DoubleArrayBuilder::arrange_from_keyset(const Keyset<T> &keyset,
                                        std::size_t begin, std::size_t end,
                                        std::size_t depth, id_type dic_id) {
    labels_.resize(0);
    value_type value = -1;
...
int DoubleArrayImpl<A, B, T, C>::build(
    ...
    const std::size_t *lengths, const value_type *values,
