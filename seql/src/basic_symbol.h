...
template <class Key, class Comp = std::less<Key>> class basic_symbol {
  public:
    typedef unsigned long hash_type;
    typedef Key key_type;

    basic_symbol() { k = &(*key_pool().insert(key_type()).first); }
    basic_symbol(const key_type &key) { k = &(*key_pool().insert(key).first); }
    basic_symbol(const basic_symbol &sym) : k(sym.k) {}
...
inline bool operator==(const basic_symbol<K, C> &x,
                       const basic_symbol<K, C> &y) {
